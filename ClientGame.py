...
def updateBoard(board): # updates board with piece states
    ...
    for x in range(8):
        for y in range(8):
            ...
            if board[x][y] == 1:
                img = pygame.image.load("white pawn.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)


            elif board[x][y] == -1:
                img = pygame.image.load("black pawn.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)
                
            elif board[x][y] == 2:
                img = pygame.image.load("white rook.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)

            elif board[x][y] == -2:
                img = pygame.image.load("black rook.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)

            elif board[x][y] == 3:
                img = pygame.image.load("white knight.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)

            elif board[x][y] == -3:
                img = pygame.image.load("black knight.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)


            elif board[x][y] == 4:
                img = pygame.image.load("white bishop.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)

            elif board[x][y] == -4:
                img = pygame.image.load("black bishop.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)


            elif board[x][y] == 5:
                img = pygame.image.load("white queen.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)

            elif board[x][y] == -5:
                img = pygame.image.load("black queen.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)


            elif board[x][y] == 6:
                img = pygame.image.load("white king.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)

            elif board[x][y] == -6:
                img = pygame.image.load("black king.png")
                imgRect = img.get_rect(center = (file, rank))
                screen.blit(img, imgRect)
...
def capturedPieces(whiteCaptured, blackCaptured):
    ...
    blackPawnIcon = pygame.image.load("black pawn.png")
    ...
    blackKnightIcon = pygame.image.load(("black knight.png"))
    ...
    blackBishopIcon = pygame.image.load(("black bishop.png"))
    ...
    blackRookIcon = pygame.image.load("black rook.png")
    ...
    blackQueenIcon = pygame.image.load(("black queen.png"))
    ...
    whitePawnIcon = pygame.image.load("white pawn.png")
    ...
    whiteKnightIcon = pygame.image.load(("white knight.png"))
    ...
    whiteBishopIcon = pygame.image.load(("white bishop.png"))
    ...
    whiteRookIcon = pygame.image.load("white rook.png")
    ...
    whiteQueenIcon = pygame.image.load(("white queen.png"))
...
def gameLoop(gameFinished, board, turn, network, state):
    ...
    while gameFinished == False:
        if state == "LANboard" and start == True:
            board = network.clientReceiveBoard()
            moveMade = network.clientReceiveMovelog()
            blackCheck = network.clientReceiveCheck()
            blackCaptured = network.clientReceiveCaptured()
        ...
        if turn == "white":
            ...
            while valid == False:
                ...
                for i in range(len(possibleMoves)):
                    ...
                    boardX = 80*arrayX + 420  # correct
                    boardY = -80*arrayY + 700 # correct

                    boardPossibleMoves.append([boardX, boardY])
                
                if displayMoves == True:
                    for i in range(len(boardPossibleMoves)):
            ...
            if state == "LANboard":
                ...
                network.clientSend(board, moveNotation, check, blackCaptured) # sending game data to opponent
                ...
                if whiteCheckmate == True:
                    blackCheckmate = detectCheckmate(board, "black", whiteKingMoved, blackKingMoved) # detects checkmate
                    if blackCheckmate == True or whiteCheck == False: # detects stalemate
                        winner = "stalemate"
                    else: # detects winner
                        winner = "black"
                ...
                # receiving game data from opponent
                board = network.clientReceiveBoard()
                moveMade = network.clientReceiveMovelog()
                blackCheck = network.clientReceiveCheck()
                whiteCaptured = network.clientReceiveCaptured()
                ...
                capturedPieces(whiteCaptured, blackCaptured)
                displayMovelog(movelogList)
                displayGameUpdate("black", False, 0, False, blackCheck, True, 0)
                pygame.display.update()


                blackCheckmate = detectCheckmate(board, "black", whiteKingMoved, blackKingMoved) # detects if black in checkmate
                
                if blackCheckmate == True:
                    whiteCheckmate = detectCheckmate(board, "white", whiteKingMoved, blackKingMoved)
                    if whiteCheckmate == True or blackCheck == False: # checks for stalemate position
                        winner = "stalemate"
                    else: # checks for if white has won
                        winner = "white"
                    gameFinished = True
                    network.clientEnd() # terminates network connection if game finished
...
while running:
    ...
    if state == "mainMenu":
        ...
        inputBox = pygame.Rect(450, 30, 500, 40)
        user_text = ""
        connected = False
        ...
        if gameFinished == False:
            ...
            user_text = "Enter Game Code - e.g '196.212.21.1'"
            connected = False
            active = False

            pygame.draw.rect(screen, colour5, inputBox) 
            text_surface = smallFont.render(user_text, True, colour2)
            screen.blit(text_surface, inputBox) # displays text input box with input instruction text
            pygame.display.flip()
            ...
            while connected == False:
                for event in pygame.event.get():
                    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                        if inputBox.collidepoint(event.pos): # detects user left click on input box
                            user_text = ""
                            active = True
                    if event.type == pygame.KEYDOWN and active == True:
                        if event.key == pygame.K_BACKSPACE:
                            user_text = user_text[:-1] # deletes last character if backspace key entered
                        elif event.key == pygame.K_RETURN:
                            IPinput = user_text
                            user_text = "Connecting to: " + IPinput
                            try:
                                network.clientConnect(IPinput) # attempts to connect to entered IP address
                            except:
                                user_text = "Invalid Game Code - Try Again" # displays error message for user, allowing them to re-enter code
                                connected = False
                                break

                            try:
                                network.clientNetworkTest() # attempts to send data to entered IP address
                            except:
                                user_text = "Invalid Game Code - Try Again"
                                network.clientEnd() # disconnects from incorrect device, sends error message for user, allowing them to re-enter code
                                connected = False
                                break

                            user_text = "Connected"
                            connected = True # ends user input loop
                            break

                        else:
                            user_text += event.unicode # displays entered character in input box
                         
                pygame.draw.rect(screen, colour5, inputBox) 
                text_surface = smallFont.render(user_text, True, colour2)
                screen.blit(text_surface, (inputBox.x+5, inputBox.y+5)) # displays updated input box state
                pygame.display.flip()

            user_text = "Connected to: " + IPinput
            text_surface = smallFont.render(user_text, True, colour2)
            screen.blit(text_surface, (inputBox.x+5, inputBox.y+5)) # displays IP address of server device, for user
            pygame.display.flip()
    ...
    for event in pygame.event.get(): # scans and handles events in game loop
        if event.type == pygame.QUIT:
            ...
            if button1Rect.collidepoint(pos): # local game option
                ...
                state = "LANboard"
            elif button3Rect.collidepoint(pos): # tutorial menu option
                state = "tutorial"
            elif button4Rect.collidepoint(pos): # settings page option
                state = "settings"
            ...
            pos = pygame.mouse.get_pos()
            if defaultColourButtonRect.collidepoint(pos):
            ...
            elif darkColourButtonRect.collidepoint(pos):
